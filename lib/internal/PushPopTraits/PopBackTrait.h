...
public:
    ...
    [[nodiscard]] constexpr reference back() const
    {
        ...
        if (pointer back = derived.try_back()) { return *back; }
        throw EmptyContainerError();
    }

    /// @brief Removes an item from the back of the container.
    /// @throws EmptyContainerError if the container is empty
    constexpr void pop_back() { if (!static_cast<Derived&>(*this).try_pop_back(1)) { throw EmptyContainerError(); } }
    ...
    constexpr bool try_pop_back() noexcept { return static_cast<Derived&>(*this).try_pop_back(1); }
